...
def train(dataset, model, optimizer, args):
    ...
    for i in range(num_batch):
        ...
        # Clamp the predictions and the Groundtruth
        pred_sdf_tensor_clamped = torch.clamp(pred_sdf_tensor, -args.clamping_distance, args.clamping_distance)
        gt_sdf_tensor = torch.clamp(gt_sdf,-args.clamping_distance, args.clamping_distance)
        # Calculate the loss for the batch
        loss = nn.L1Loss(reduction='sum')
        loss_l1 = loss(pred_sdf_tensor,gt_sdf_tensor)
        average_loss_l1 = loss_l1/xyz_tensor.shape[0]
        # Update loss sum and loss counts
        average_loss_l1.backward()
        optimizer.step()
        loss_sum   += loss_l1.detach()
...
def val(dataset, model, optimizer, args):
    ...
    for i in range(num_batch):
        ...
        # **** YOU SHOULD ADD TRAINING CODE HERE, CURRENTLY IT IS INCORRECT ****
        with torch.no_grad():
            # Load XYZ tensor and corresponding ground truths 
            xyz_tensor = data['xyz'].to(device)
            gt_sdf = data['gt_sdf'].to(device)
            # Get predictions from the model 
            pred_sdf_tensor = model(xyz_tensor)
